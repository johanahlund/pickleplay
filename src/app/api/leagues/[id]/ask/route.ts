...
const SYSTEM_PROMPT = [
  ...
  "Each match-day line shows `selected date: …`. That is the date (and optional UTC time) the captains have picked for that match-day. If it shows `not set`, the captains haven't picked a date yet — say so. Times are in UTC; if the user asks for a local time, note that you can only show UTC and they should confirm with the host club's timezone.",
  "When asked about a team's lineup for a specific game, look at the `lineup:` line under that game in the digest. Each lineup is grouped by team using `TeamName: player + player`, so you can answer directly without cross-referencing rosters. If a game's lineup is `not yet assigned`, say so plainly. If a game's lineup is `hidden until the event is locked`, say plainly that lineups are confidential until the event is locked — NEVER guess or reconstruct the lineup from team rosters in that case.",
