...
export function StepUpperBracket({ config, canManage, updateConfig }: StepUpperBracketProps) {
  const numAdvancing = config.numGroups * config.advanceToUpper + config.wildcardCount;
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-4 space-y-4">
        ...
        <div className="space-y-1.5">
          {stages.map((stage) => (
            <div key={stage} className="flex items-center gap-2">
              ...
              <select
                disabled={!canManage}
                value={config.upperBracketFormats[stage] || "to_11"}
                onChange={(e) => {
                  const newFormats = { ...config.upperBracketFormats };
                  newFormats[stage] = e.target.value;
                  // Cascade: set later stages to same if not already set
                  const stageIdx = stages.indexOf(stage);
                  for (let i = stageIdx + 1; i < stages.length; i++) {
                    if (!config.upperBracketFormats[stages[i]]) {
                      newFormats[stages[i]] = e.target.value;
                    }
                  }
                  updateConfig({ upperBracketFormats: newFormats });
                }}
      ...
      <label className="flex items-center gap-2 text-sm">
        <input type="checkbox" checked={config.upperThirdPlace}
          disabled={!canManage}
