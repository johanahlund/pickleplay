...
export function eventStatusBadgeClass(
  ...
  // Closed: date-derived phase decides the colour.
  //   scheduled (future)   → emerald (event published, on the way)
  //   in_progress (today)  → green (it's happening)
  //   completed (past)     → grey
  //   pure closed (no date)→ orange ("signups locked, no schedule")
